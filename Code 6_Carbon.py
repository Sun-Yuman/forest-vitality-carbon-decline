...
data['survey_year'] = pd.to_numeric(data['survey_year'], errors='coerce')
data['Net CO2 emissions/kt_ctrybase%'] = pd.to_numeric(data['Net CO2 emissions/kt_ctrybase%'], errors='coerce')
data['Net change_base%'] = pd.to_numeric(data['Net change_base%'], errors='coerce')
data.dropna(subset=['survey_year', 'Net CO2 emissions/kt_ctrybase%', 'Net change_base%'], inplace=True)
...
ax.fill_between(X_sorted.ravel(), pred_all['mean_ci_lower'], pred_all['mean_ci_upper'], color='#0066CC', alpha=0.2)
...
ax.text(0.18, 0.90,
        ...
        transform=ax.transAxes, fontsize=16, va='bottom', color='#0066CC')
...
if d1021.shape[0] >= 2:
    ...
    ax.plot(X2_sorted.ravel(), yhat2, '-', color='#8A2BE2', linewidth=1.5)  # 紫色
    ax.fill_between(X2_sorted.ravel(), pred2['mean_ci_lower'], pred2['mean_ci_upper'], color='#8A2BE2', alpha=0.2)
    ...
    ax.text(0.18, 0.85,
            ...
            transform=ax.transAxes, fontsize=16, va='bottom', color='#8A2BE2')
...
out_path = os.path.join(BASE_DIR, '3_net_change_and_defdev.pdf')
...
def calculate_loess_confidence(x, y, frac=0.66, n_boot=10000, confidence=0.95):
    """
    Perform LOESS smoothing and calculate confidence intervals using Bootstrap resampling.
    x: Independent variable (e.g., years)
    y: Dependent variable (observed data)
    frac: LOESS smoothing parameter (increased for smoother fits)
    n_boot: Number of bootstrap resampling iterations
    confidence: Confidence level for intervals (e.g., 0.95 for 95% CI)
    """
    # Remove missing values
    mask = ~np.isnan(y)
    x = x[mask].values  # Convert to numpy array
    y = y[mask].values  # Convert to numpy array
    ...
    for i in range(n_boot):
        ...
        # Apply LOESS on bootstrap sample
        bs_fit = lowess(y_bs, x_bs, frac=frac)
        # Interpolate the bootstrap fitted results to the original x values
        y_boot[i, :] = np.interp(x, bs_fit[:, 0], bs_fit[:, 1])

    # Calculate confidence intervals based on the bootstrap resampling
    lower_percentile = (1 - confidence) / 2 * 100  # Calculate the lower percentile
    upper_percentile = (1 + confidence) / 2 * 100  # Calculate the upper percentile

    # Calculate the lower and upper confidence intervals
    ci_lower = np.percentile(y_boot, lower_percentile, axis=0)
    ci_upper = np.percentile(y_boot, upper_percentile, axis=0)

    # Calculate R^2 and residuals
    y_pred = np.interp(x, smoothed[:, 0], y_fit)
    r2 = r2_score(y, y_pred)
    residuals = y - y_pred
    std = np.std(residuals)

    return y_fit, ci_lower, ci_upper, residuals, r2, std

# Prepare data by dropping missing values
data = data.dropna(
    subset=['survey_year', 'Net change_base%', 'mean_defoliation_base'])
x = data['survey_year']
...
net_change_fit, net_change_ci_lower, net_change_ci_upper, net_change_residuals, net_change_r2, net_change_std = calculate_loess_confidence(
...
defoliation_fit, defoliation_ci_lower, defoliation_ci_upper, defoliation_residuals, defoliation_r2, defoliation_std = calculate_loess_confidence(
...
# Save the figure
plt.savefig(BASE_DIR + '4_NetC02_def_Netbiomass_loessfit.jpg', dpi=300)
...
# ===================== COLORS =====================
PALETTE = {
    "1990_pos": (160 / 255, 190 / 255, 245 / 255, 1.0),  # light blue (#A0BEF5)
    "1990_neg": (160 / 255, 190 / 255, 245 / 255, 1.0),  # light blue (#A0BEF5)
    "2010_pos": (157 / 255, 43 / 255, 194 / 255, 1.0),  # deep purple (#9D2BC2)
    "2010_neg": (157 / 255, 43 / 255, 194 / 255, 1.0),  # deep purple (#9D2BC2)
}
