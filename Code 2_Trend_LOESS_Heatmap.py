...
- `add_loess_fit(ax, x, y, frac=0.3, it=0, degree=1)`: Adds a LOESS (locally weighted regression) smooth line and reports R², standard deviation, and confidence intervals.
...
def add_loess_fit(ax, x, y, frac=0.3, it=0, n_boot=400, seed=42):
    """Add a LOESS（lowess） fit line to a plot and calculate R-squared, standard deviation, and confidence intervals."""
    x = np.asarray(x)
    y = np.asarray(y)
    x_grid = np.arange(1990, 2023)
    lo = lowess(y, x, frac=frac, it=it, return_sorted=True)
    x_fit, y_fit_sorted = lo[:, 0], lo[:, 1]
    y_fit = np.interp(x_grid, x_fit, y_fit_sorted)
    y_on_obs = np.interp(x, x_grid, y_fit)
    r2 = r2_score(y, y_on_obs)
    residuals = y - y_on_obs
    std = float(np.std(residuals, ddof=1))

    ax.plot(x_grid, y_fit, label='LOESS fit', color='#2F4F4F')

    rng = np.random.default_rng(seed)
    boot_mat = np.empty((n_boot, x_grid.size), dtype=float)
    ...
    for b in range(n_boot):
        ...
        lo_b = lowess(yb, xb, frac=frac, it=it, return_sorted=True)
        x_bfit, y_bfit_sorted = lo_b[:, 0], lo_b[:, 1]
        try:
            y_bfit = np.interp(x_grid, x_bfit, y_bfit_sorted)
        except Exception:
            y_bfit = np.full_like(x_grid, np.nan, dtype=float)

        boot_mat[b, :] = y_bfit

    y_lo = np.nanpercentile(boot_mat, 2.5, axis=0)
    y_hi = np.nanpercentile(boot_mat, 97.5, axis=0)

    ax.fill_between(x_grid, y_lo, y_hi, color='lightgrey', alpha=0.8, label='95% Confidence Interval')
    ax.text(0.3, 0.60, f'$R^2$ = {r2:.2f}\n Standard deviation = {std:.2f}',
            transform=ax.transAxes, fontsize=17, ha='right', va='bottom', color='black')

def combined_trend_plot_segmented_without_mk(df, ax1, ax2, column_name):
    """Plot segmented trend lines for different time periods."""
    periods = [(1990, 1999, '#000000'), (2000, 2009, '#000000'), (2010, 2019, '#000000')]
    ...
    x_obs = np.arange(1990, 2023)
    y_obs = np.interp(x_obs, full_x, full_y)
    add_loess_fit(ax1, x_obs, y_obs, frac=0.3, it=0, n_boot=400, seed=42)
...
def combined_plot_with_bioregion_heatmap(df, columns_list, dpi=300):
    ...
    file_path = os.path.join(BASE_DIR, "Trend_LOESS_Heatmap.jpg")
...
def collect_trend_data(df, column_name, region=None):
    ...
    results = []
    #Time period color distinction is not shown on the Figure
...
color_mapping = {
    "All species": "#4D4D4D",   # Gray
    "Broadleaves": "#D95F02",   # Orange
    "Conifers": "#7570B3",      # Blue-purple
    "Beech": "#A6761D",         # Brown
    "Oak": "#E7298A",           # Magenta
    "Norway spruce": "#6A3D9A", # Purple
    "Scots pine": "#E6AB02"     # Yellow-brown
}
...
plt.savefig('trend_region_periods.jpg', dpi=300)
plt.show()
